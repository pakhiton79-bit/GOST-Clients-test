...
function computeGost10198I3(input){
  ...
  // сосуществуют, не заменяют друг друга). Отдельно: k10<=0 (длина/ширина груза
  // выбраны так, что вычесть 400мм уже не из чего - типичный признак того, что
  // длина и ширина перепутаны местами) - это не блокирующая ошибка (findNegativeField
  // не ловит ровно 0, только отрицательные), но показывать в таблице "0 мм" как
  // будто это настоящий размер детали нельзя - вместо чисел тоже выводится ⚠,
  // не дожидаясь галочки "Погрузка погрузчиком" (по указанию пользователя).

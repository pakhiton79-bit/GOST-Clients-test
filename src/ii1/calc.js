...
function computeGost10198II1(input){
  ...
  for(let iter=0; iter<4; iter++){
    ...
    // крышки - включая отступы от краёв, которые равны промежуткам между
    // соседними брусьями (а не flush-edge, как у стоек каркаса) - по прямому
    // уточнению пользователя. При n брусьях промежутков (n+1) штук, значит
    // шаг осей = (L+w21)/(n+1) (вывод: см. комментарий у edgeDistCross ниже) -
    // отсюда минимальное n, при котором шаг осей ещё ≤700мм.
    crossBeamCount = Math.max(2, Math.ceil((L + w21) / 700 - 1));
    // Отступ от края крышки до края крайнего бруса при таком n не должен
    // быть меньше толщины обшивки + толщины стойки + 10мм зазора (по
    // уточнению пользователя) - иначе брус упрётся в каркас торцевого щита.
    // Меньше брусьев -> больше отступ, поэтому минимальное n (выбранное выше
    // из условия шага осей) даёт МАКСИМАЛЬНО возможный отступ - если даже он
    // не проходит по минимуму, снизить его больше нельзя (меньше n нарушит
    // шаг осей ≤700мм) - остаётся только предупредить.
    ...
    if(lidLayout === 'transverse'){
      ...
      // Шаг осей поперечного бруса - той же формулой, что и crossBeamCount
      // выше (равномерное распределение, включая отступы от краёв), а не
      // clearGapBySpan (flush-edge, как у стоек) - чтобы сечение продольного
      // бруса (longBeamSection) считалось по РЕАЛЬНОМУ шагу осей.
  ...
  // (см. комментарий там) - по методике I-3 (diagramKryshkaDefault/
  // diagramKryshka2Beams, src/diagrams.js), подтверждено пользователем.
  ...
  // Толщина/ширина/количество всегда считаются и показываются как обычно (по
  // уточнению пользователя) - от длины они не зависят и сами по себе верны,
  // даже если длина получилась недопустимой; в таблице ⚠ ставится ТОЛЬКО в
  // колонке длины - либо когда k10<=0 (длина/ширина груза выбраны так, что
  // вычесть 400мм уже не из чего - типичный признак того, что длина и ширина
  // перепутаны местами; findNegativeField не ловит ровно 0, только
  // отрицательные), либо когда включена "Погрузка авто/электропогрузчиком" и
  // требование ≥300мм не выполняется.
  ...
  // реально укорачивает длину внутреннего поперечного бруса на 4мм - и (по
  // уточнению пользователя, отменяющему более раннее решение "только
  // поперечный") длину внутреннего продольного бруса тоже на 4мм.
  ...
  // sideFrameDisplay - та же косметическая надбавка +2мм на чертеже крышки
  // для подписи "толщина стойки + толщина обшивки бока" (t_stojka+skin.value)
  // при включённой «Оптимизировать размеры» - по уточнению пользователя, по
  // аналогии с t32Display выше. На реальные t_stojka/skin.value (идут в
  // таблицу деталей, наружные размеры и т.д.) не влияет.
...
function calculate(){
  ...
  // Щит торцевой/боковой: 260 (вместо прежних 210) - тот же базовый размер,
  // что и у Дна/Крышки (DIAGRAM_DEFAULT_WIDTH), по замечанию пользователя
  // о том, что эти два чертежа должны быть увеличены, как остальные. 0.8 -
  // labelScale (см. renderDiagram() в src/common-diagrams.js): фиксированный
  // множитель подписей независимо от входных размеров груза - авто-сжатие
  // через --dk у этих двух чертежей срабатывает непредсказуемо слабо (мало

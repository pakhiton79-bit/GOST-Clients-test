...
function selectSkid19(mass, workingLengthMm, widthMm){
  ...
  const options = massRow.rows.map(row=>{
    ...
    // таблице «Новые стандарты полозьев.docx» (столбец "Высота и ширина
    // полозьев": первое число ячейки - высота/толщина, второе - ширина).
    // Раньше (уточнение пользователя) такое позиционное чтение считалось
    // ненадёжным - в части ячеек толщина там больше ширины (напр. "100x75",
    // "175x150" и т.п., 66 из 286), и толщину/ширину принудительно
    // переставляли (меньшее=толщина, большее=ширина). По новому уточнению
    // пользователя таблицу нужно читать максимально точно - без перестановки,
  ...
  if(valid.length===0){
    ...
    // у масс 500-4000кг в Табл.19 предусмотрено только 2/3 полоза). По
    // уточнению пользователя: добавляем максимум 1 полоз сверх табличного
    // варианта с БОЛЬШИМ количеством, БЕЗ понижения сечения (то же сечение,
    // что и у табличного варианта) - вопреки тексту примечания к самой
    // таблице («на одну градацию ниже»), которое здесь не применяется. Если
    // даже +1 не хватает - добавляем ещё (тем же сечением), но это уже повод
    // для предупреждения.
...
// всё равно выходит за 75-99мм — используем как есть и сообщаем через .warn (производственная
// практика, согласовано с конструктором, вне текста ГОСТа).
...
function fillBoards(space, roundWidths){
  ...
  if(roundWidths){
    ...
    // дополнительных досок 75-99мм; суммарная ширина досок при этом может
    // выйти за пределы доступного места - лишнее у последней доски подрежут
    // на месте (см. текст подсказки в shell.html/calc.src.html).
